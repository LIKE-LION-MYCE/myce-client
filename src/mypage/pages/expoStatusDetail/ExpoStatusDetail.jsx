...
const getStatusLabel = (status, t) => {
  return t(`mypageGeneral.expoStatus.status.${status}`) || status;
};
...
const ExpoStatusDetail = () => {
  ...
  const fetchExpoDetail = async () => {
    try {
      ...
      const transformedData = {
        ...
        name: data.title || t('mypageGeneral.expoStatus.detail.defaultValues.noTitle'),
        location: data.locationDetail ? `${data.location} (${data.locationDetail})` : data.location || t('mypageGeneral.expoStatus.detail.defaultValues.noLocation'),
        ...
        category: data.category || t('mypageGeneral.expoStatus.detail.defaultValues.noCategory'),
        description: data.description === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noDescription') : (data.description || t('mypageGeneral.expoStatus.detail.defaultValues.noDescription')),
        ...
        companyName: data.businessInfo?.companyName === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noCompanyName') : (data.businessInfo?.companyName || t('mypageGeneral.expoStatus.detail.defaultValues.noCompanyName')),
        companyAddress: data.businessInfo?.address === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noAddress') : (data.businessInfo?.address || t('mypageGeneral.expoStatus.detail.defaultValues.noAddress')),
        businessRegistrationNumber: data.businessInfo?.businessRegistrationNumber === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noBusinessNumber') : (data.businessInfo?.businessRegistrationNumber || t('mypageGeneral.expoStatus.detail.defaultValues.noBusinessNumber')),
        ceoName: data.businessInfo?.ceoName === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noCeoName') : (data.businessInfo?.ceoName || t('mypageGeneral.expoStatus.detail.defaultValues.noCeoName')),
        ceoContact: data.businessInfo?.contactPhone === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noContact') : (data.businessInfo?.contactPhone || t('mypageGeneral.expoStatus.detail.defaultValues.noContact')),
        ceoEmail: data.businessInfo?.contactEmail === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noEmail') : (data.businessInfo?.contactEmail || t('mypageGeneral.expoStatus.detail.defaultValues.noEmail')),
        applicantName: data.businessInfo?.ceoName === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noApplicant') : (data.businessInfo?.ceoName || t('mypageGeneral.expoStatus.detail.defaultValues.noApplicant')),
        memberLoginId: data.memberLoginId || t('mypageGeneral.expoStatus.detail.defaultValues.noLoginId'),
        ...
        tickets: data.tickets?.map(ticket => ({
          ...
          name: ticket.name === 'string' ? t('mypageGeneral.expoStatus.detail.defaultValues.noTicketName') : ticket.name
        })) || [],
      ...
      setError(t('mypageGeneral.expoStatus.detail.error'));
      setExpoData(null);
    } finally {
      setLoading(false);
    }
  ...
  const handleOpenModal = async () => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.paymentInfoError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePay = async () => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.paymentCompleted'));
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.paymentFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAdminModal = async () => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.adminInfoError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancelExpo = async () => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.expoCancelled'));
      window.history.back(); // 이전 페이지로 돌아가기
    } catch (err) {
      console.error('박람회 취소 실패:', err);
      alert(t('mypageGeneral.expoStatus.detail.messages.expoCancelFailed'));
    }
  };
  ...
  const handleRefundButtonClick = async () => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.refundInfoError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRefund = async (refundReason) => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.refundCompleted'));
      ...
      if (err.response?.data?.errorCode === 'RF003') {
        ...
        alert(t('mypageGeneral.expoStatus.detail.messages.refundFailed'));
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSettlementRequest = async () => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.settlementInfoError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleActualSettlementRequest = async (bankInfo) => {
    try {
      ...
      alert(t('mypageGeneral.expoStatus.detail.messages.settlementCompleted'));
      handleCloseSettlementReceiptModal();
      fetchExpoDetail(); // 상태 업데이트를 위해 데이터 다시 불러오기
    } catch (err) {
      console.error('정산 요청 실패:', err);
      alert(t('mypageGeneral.expoStatus.detail.messages.settlementFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className={styles.container}>
        <div className={styles.loading}>{t('mypageGeneral.expoStatus.detail.loading')}</div>
      </div>
    );
  }
  ...
  if (!expoData) {
    return (
      <div className={styles.container}>
        <div className={styles.loading}>{t('mypageGeneral.expoStatus.detail.notFound')}</div>
      </div>
    );
  }
    ...
    <div className={styles.container}>
      ...
      {showPaymentInfoModal && paymentInfoData && (
        ...
        >
          ...
          >
            {t('mypageGeneral.expoStatus.modal.confirm')}
          </button>
        </PaymentDetailModal>
      )}
