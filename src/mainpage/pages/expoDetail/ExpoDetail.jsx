  ...
  const handleBookmarkToggle = async (e) => {
    ...
    try {
      ...
      if (result.isBookmarked) {
        showSuccessMessage(
          t(
            "expoDetail.expoDetailMain.alerts.bookmarkAdded",
            "북마크에 추가되었습니다."
          )
        );
      } else {
        showSuccessMessage(
          t(
            "expoDetail.expoDetailMain.alerts.bookmarkRemoved",
            "북마크에서 제거되었습니다."
          )
        );
      }
      ...
      if (status === 401 || err.message === "LOGIN_REQUIRED") {
        showFailMessage(
          t(
            "expoDetail.expoDetailMain.alerts.loginRequired",
            "로그인이 필요한 서비스입니다."
          )
        );
        ...
        showFailMessage(
          t(
            "expoDetail.expoDetailMain.alerts.bookmarkError",
            "찜하기 처리에 실패했습니다."
          )
        );
      }
      ...
      <div className={styles.pageWrapper}>
        <div className={styles.container}>
          <div className={styles.error}>
            {t(
              "expoDetail.expoDetailMain.errors.expoNotFound",
              "박람회 정보를 찾을 수 없습니다."
            )}
          </div>
        </div>
    ...
    <div className={styles.pageWrapper}>
      <div
        className={`${styles.container} ${
          isPendingPublish ? styles.pendingPublish : ""
        }`}
      >
        {/* 뒤로가기 버튼 */}
        <div className={styles.backButtonSection}>
          <button className={styles.backButton} onClick={handleGoBack}>
            <FiArrowLeft size={20} />
            <span>
              {t("expoDetail.expoDetailMain.navigation.back", "이전으로")}
            </span>
          </button>
        </div>
        ...
        <div className={styles.tabs}>
          ...
          >
            {t("expoDetail.expoDetailMain.tabs.booths", "부스 정보")} (
            {booths?.length || 0})
          </button>
          <button
            className={`${styles.tab} ${
          ...
          >
            {t("expoDetail.expoDetailMain.tabs.events", "이벤트")} (
            {events?.length || 0})
          </button>
          <button
            className={`${styles.tab} ${
          ...
          >
            {t("expoDetail.expoDetailMain.tabs.reviews", "리뷰")} (
        ...
        <div className={styles.tabContent}>
          {activeTab === "info" && (
            <ExpoInfo basicInfo={basicInfo} location={location} />
          )}

          {activeTab === "tickets" && <ExpoTickets tickets={tickets} />}

          {activeTab === "booths" && <ExpoBooths booths={booths} />}
        ...
        {/* 채팅 모달 */}
        <ChatModal isOpen={showChatModal} onClose={handleChatClose} />
