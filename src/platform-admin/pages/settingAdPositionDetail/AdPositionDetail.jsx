...
function AdPositionDetail() {
  ...
  const runValidation = () => {
    ...
    else if (isNaN(Number(f.bannerWidth)))
      e.bannerWidth = "이미지 너비는 숫자여야 합니다.";
    else if (Number(f.bannerWidth) <= 0)
      e.bannerWidth = "이미지 너비는 1 이상이어야 합니다.";

    if (isBlank(f.bannerHeight))
      e.bannerHeight = "이미지 높이(px)는 필수입니다.";
    else if (isNaN(Number(f.bannerHeight)))
      e.bannerHeight = "이미지 높이는 숫자여야 합니다.";
    else if (Number(f.bannerHeight) <= 0)
      e.bannerHeight = "이미지 높이는 1 이상이어야 합니다.";

    if (isBlank(f.maxBannerCount))
      e.maxBannerCount = "최대 광고 개수는 필수입니다.";
    else if (isNaN(Number(f.maxBannerCount)))
      e.maxBannerCount = "최대 광고 개수는 숫자여야 합니다.";
    else if (Number(f.maxBannerCount) < 1)
      e.maxBannerCount = "최대 광고 개수는 1 이상이어야 합니다.";
    ...
    try {
      const payload = {
        ...
        formData: {
          ...formData,
          bannerWidth:
            formData.bannerWidth === "" ? null : Number(formData.bannerWidth),
          bannerHeight:
            formData.bannerHeight === "" ? null : Number(formData.bannerHeight),
          maxBannerCount:
            formData.maxBannerCount === ""
              ? null
              : Number(formData.maxBannerCount),
        },
      };
        ...
        <form onSubmit={handleSubmit}>
          <div className={styles.formFlat}>
            <div className={styles.formGroup}>
              ...
              />
              {errors.bannerName && (
                <p className={styles.errorText}>{errors.bannerName}</p>
              )}
            </div>
            ...
            <div className={styles.formGroup}>
              ...
              />
              {errors.bannerWidth && (
                <p className={styles.errorText}>{errors.bannerWidth}</p>
              )}
            </div>
            ...
            <div className={styles.formGroup}>
              ...
              />
              {errors.bannerHeight && (
                <p className={styles.errorText}>{errors.bannerHeight}</p>
              )}
            </div>
