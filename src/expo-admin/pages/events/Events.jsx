...
function Events() {
  ...
  const pagedData = eventList.slice(page * size, (page + 1) * size);
  const pageInfo = {
    totalPages: Math.ceil(eventList.length / size),
    number: page,
  };

  const handleAdd = (event) => {
    const newId = eventList.length ? Math.max(...eventList.map((e) => e.id)) + 1 : 1;
    setEventList((prev) => [...prev, { ...event, id: newId }]);
  };
